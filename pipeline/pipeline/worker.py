...
def run_pipeline(doc_id: str):
    """
    S0 (placeholder) -> S1 -> S2 (placeholder)
    Замерь время каждого этапа и сохрани артефакты.
    """
    ...
    try:
        # ---- S0 (заглушка, если файл уже готов) ----
        t0 = time.time()
        if not s0_path.exists():
            raise FileNotFoundError(f"S0 not found: {s0_path}")
        time.sleep(0.1)  # имитация работы
        _push_stage(doc_id, "S0", t0, notes="loaded S0")
        ...
        # ---- S2 (placeholder) ----
        t2 = time.time()
        # здесь можно добавить MiniLM/FAISS; пока имитируем
        time.sleep(0.05)
